...
CatCan.prototype.init = function(divId/*string*/,animation/*object*/) {//初始化函数
...
var CatCanTool = CatCan.prototype.tool = {//工具集
	/*
		...
		返回值:
			{
				"data": data,//完全空白的数据域,可以随意往里面存些东西
				"fun": fun//String待执行的函数体字符串,初始化时会转化为可执行的函数,该函数会在每次打印这个图层时执行
			};
		...
		函数编写规范:
			...
			3.返回结果
		其他:
			1.无论工具集函数何时被调用,都会优先于动画配置文件内其他代码段执行,如drEll函数的原理是修改this[2]和this[3],则按如下两条配置的图层虽然不会报错,却都会静止不动
				[0,1,900,0,"this[2]=0;this[3]=0;drEll(100,50,1);"]
				[0,1,900,0,"drEll(100,50,1);this[2]=0;this[3]=0;"]
			2.可以仿照示例中做的那样在代码段的最前面加一句"var df=this.data["+f+"];",df与返回值中data指向同一地址空间,以此来方便地调用数据域中的数据
	*/
};