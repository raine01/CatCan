...
CatCan.prototype.getData = function(listNo/*number*/,aniNo/*number*/,funNo/*number*/) {//获取数据域
	if(typeof listNo==="object") {//如果传入的是个
		funNo = listNo.funNo;
		aniNo = listNo.aniNo;
...
CatCan.prototype.query = function(str/*string*/) {//获取str函数何处使用
	/*
		参数列表:
			str:函数名
	*/
	...
	for(var i in this.catTreePlay) {
		...
		for(var j in ani) {
			if(ani[j][4].indexOf(str)==-1) continue;
			var a = ani[j][4].match(regexp);
			for(var k in a) {
...
var CatTool = CatCan.prototype.tool = {//工具集
	/*
		函数工具集约定/提倡如下规范:
		函数命名符合如下正则表达式,
		...
		返回值:
			{
				data: data,//完全空白的数据域,可以在text中通过this.data[f]或在任意代码空间通过CatCan.getData()方法访问
				text: text,//String待执行的函数体字符串,初始化时会转化为可执行的函数,该函数会在每次打印这个图层时执行
				fun: fun//function,会在text后执行,如有多个动画共用该图层,这种方法可能会造成动画冲突
			};
		...
		函数编写规范:
			...
			3.返回结果
		其他:
			1.绘制图层前函数的调用顺序为1)文本类预处理函数2)字符串编译函数3)函数类预处理函数
			2.预处理函数何时被调用与写在何处无关,但是我们建议
